...
public class InformePanel extends JPanel implements ActionListener {
    ...
    public InformePanel(JFrame ventanaInforme) {
        this.ventanaInforme = ventanaInforme;
        ...
        tblPiezas.setModel(new DefaultTableModel(
                new Object[][]{},
                new String[]{
                    "Clave", "Nombre", "Precio", "Piezas disponibles", "Minimo de piezas"
                }
        ));
        ...
        Border bordejpanel = new TitledBorder(new EtchedBorder(), "Piezas por agotarse");
        ...
        if (validacion.hayPiezas()) {
            for (Pieza pieza : validacion.getPiezas()) {
                if (pieza.getUnidades() <= pieza.getAlerta()) {
            ...
            if (contador <= 0) {
                JOptionPane.showMessageDialog(null, "No hay piezas por agotarse.", "Correcto", JOptionPane.INFORMATION_MESSAGE, new ImageIcon("src/Img/correcto.png"));
            }
            lblCantidadPiezas.setText("Piezas: " + contador);
        }
