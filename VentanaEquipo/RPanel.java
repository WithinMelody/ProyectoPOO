...
public class RPanel extends JPanel implements ActionListener {
    ...
    public RPanel(JFrame ventanaReporte) {
        ...
        tblPiezas.setModel(new DefaultTableModel(
                new Object[][]{},
                new String[]{
                    "Numero de equipo", "Materia", "Profesor", "Nombre del equipo", "Numero de integrantes"
                }
        ));
        ...
        Border bordejpanel = new TitledBorder(new EtchedBorder(), "Reporte de Equipos");
        setBorder(bordejpanel);

        //Agregando datos a la tabla
        ValEquipo validacion = new ValEquipo();
        long contador = 0;
        if (validacion.hayEquipo()) {
            for (Equipo eq : validacion.getEquipos()) {
                tabla = (DefaultTableModel) tblPiezas.getModel();
                Object[] fila = new Object[tabla.getColumnCount()];
                fila[0] = eq.getNumequipo();
                fila[1] = eq.getMateria();
                fila[2] = eq.getProfesor();
                fila[3] = eq.getNombreE();
                fila[4] = eq.getNumintegrantes();
                tabla.addRow(fila);
                contador++;
            }
            lblCantidadPiezas.setText(" Numero de equipos: " + contador);
        }
